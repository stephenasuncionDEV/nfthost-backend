    ...
    return new Promise((resolve) => {
        try {
            dns.resolveAny(domain, (err, records) => {
                if (err || !records) return resolve({ message: "Cannot find domain's DNS records.", status: false });

                const aliases = records.filter((record) => record.type === 'A');
                if (!aliases) return resolve({ message: 'Please ask staff to verify your custom domain on Discord.', status: false });

                if (!aliases[0].address.startsWith('76.76.21.')) return resolve({ message: 'Please ask staff to verify your custom domain on Discord.', status: false });
